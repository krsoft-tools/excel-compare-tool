...
async def export_excel(
    ...
    df1 = pd.read_excel(file1.file).fillna("")
    df2 = pd.read_excel(file2.file).fillna("")
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        ...
        if tag == "equal":
            ...
            for k in range(length):
                r2 = df2.iloc[j1 + k].tolist() if j1 + k < j2 else [""] * len(df2.columns)
    ...
    result_df = pd.DataFrame(output_rows, columns=df2.columns)
    ...
    for idx, row in enumerate(ws.iter_rows(min_row=2), start=0):
        ...
        if row_types[idx] == "replace":
            for col_idx, cell in enumerate(row):
                val2 = cell.value

                # z df1 vezmeme pôvodnú hodnotu
                ...
                if str(val1) != str(val2):
                    ...
                    ws_changes.cell(row=row_num, column=1).value = f"{col_letter}{idx+2}: {val1} → {val2}"
                    row_num += 1
